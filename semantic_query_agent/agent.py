...
async def call_interpret(messages: list[BaseMessage], system_prompt: str, llm: ChatOpenAI) -> InterpretResult:
...
async def call_clarify(messages: list[BaseMessage], ambiguity_reason: str, llm: ChatOpenAI) -> str:
...
async def call_respond(
    messages: list[BaseMessage], query_result: list[dict] | None, error: str | None, llm: ChatOpenAI
) -> str:
...
def create_agent(
    ...
    system_prompt = build_interpret_system_prompt(semantic_model)
    retries = max_validation_retries if max_validation_retries is not None else settings.max_validation_retries
    ...
    async def interpret_node(state: AgentState) -> dict:
        ...
        result = await call_interpret(messages, system_prompt, llm)
    ...
    def route_node(state: AgentState) -> dict:
        ...
        if result.query_plan:
            error = _validate_query_plan(result.query_plan, semantic_model)
            if error:
    ...
    async def clarify_node(state: AgentState) -> dict:
        ...
        response = await call_clarify(list(state.messages), state.interpret_result.ambiguity_reason, llm)
