...
def build_sql(plan: QueryPlan, model: SemanticModel) -> tuple[str, list]:
    """Build a DuckDB SQL query from a QueryPlan and SemanticModel.

    Returns a (sql, params) tuple with parameterized filter values.
    """
    ...
    for dim_name, dim_value in plan.filters.items():
        ...
        dim = dimensions_map[dim_name]
        where_parts.append(f"{_translate_expr(dim.expr)} = ?")
